...
// Advanced Login Pro actions

/**
 * Filtra las respuestas HTTP para asegurar que sean del tipo correcto en solicitudes AJAX
 */
function wp_alp_fix_ajax_content_type() {
    // Solo modificar para solicitudes AJAX
    if (
        defined('DOING_AJAX') && DOING_AJAX && 
        isset($_POST['action']) && 
        strpos($_POST['action'], 'wp_alp_') === 0 &&
        isset($_POST['is_ajax']) && 
        $_POST['is_ajax'] === 'true'
    ) {
        ...
        // Prevenir cualquier salida antes de la respuesta JSON
        ob_start();
        add_action('shutdown', function() {
            $output = ob_get_clean();
            
            // Si hay salida previa, guardarla en un log y entregar una respuesta JSON de error
            if (!empty($output)) {
                error_log('WP ALP - Unexpected output before JSON response: ' . $output);
...
add_action('init', 'wp_alp_fix_ajax_content_type', 1);

/**
 * Prevenir redirecciones durante solicitudes AJAX de WP Advanced Login Pro
 */
function wp_alp_prevent_ajax_redirects() {
    if (
        defined('DOING_AJAX') && DOING_AJAX && 
        isset($_POST['action']) && 
        strpos($_POST['action'], 'wp_alp_') === 0 &&
        ...
        // También desactivar otras formas de redirección
        add_filter('wp_safe_redirect', '__return_false', 999);
        remove_all_actions('wp_logout');
        
        // Capturar headers de redirección
...
add_action('init', 'wp_alp_prevent_ajax_redirects', 1);

/**
 * Añadir el atributo 'is_ajax' a los formularios de login y registro
 */
function wp_alp_add_ajax_flag_to_forms($form_html) {
    // Añadir campo oculto para indicar que es una solicitud AJAX
    $ajax_flag = '<input type="hidden" name="is_ajax" value="true">';
    
    // Insertar antes del cierre del formulario
    $form_html = str_replace('</form>', $ajax_flag . '</form>', $form_html);
    
    return $form_html;
}
// Aplicar a todos los formularios generados por shortcodes
add_filter('wp_alp_login_form', 'wp_alp_add_ajax_flag_to_forms');
add_filter('wp_alp_register_user_form', 'wp_alp_add_ajax_flag_to_forms');
add_filter('wp_alp_register_vendor_form', 'wp_alp_add_ajax_flag_to_forms');
add_filter('wp_alp_profile_completion_form', 'wp_alp_add_ajax_flag_to_forms');